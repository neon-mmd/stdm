...
void search_the_manual(str ask, int lines, str launcher, str searchTerm, str path)
{
  str txt;
  sys(("apropos " + str(ask) + " | " + launcher + " | cut -d ' ' -f1 > " + path + "temp.txt").c_str());
  ...
  temp2.close();
  sys(("cat " + path + "temp2.txt | " + launcher + " | cut -d ' ' -f1 > " + path + "temp3.txt").c_str());
  ...
  Table table;
  table.add_row({"The following excerpt was taken from line " + to_string(startLine) + " to " + to_string(endLine)});
  table.add_row({txt2});
  table.format()
    .padding(1)
...
void usage()
{ 
  cout << "Usage: " << "stdm" << " [options] " << "<name>" << "\n";
  cout << "\n";
  cout << "\t-h" << "\tprovide this help list" << "\n";
  cout << "\t-f" << "\tdisplay the full manual with example manual if available" << "\n";
  cout << "\t-s" << "\tsearch the manual page with the functionality" << "\n";
  cout << "\t-e" << "\tgive the example manual if available" << "\n";
  cout << "\t-l" << "\tthe number of lines of excerpt to display from the manual (default: 5)" << "\n";
  cout << "\t-L" << "\tprovide an appropriate launcher (default: fzf)" << "\n";
  cout << "\t-S" << "\tsearch the manual by providing a search term should be used with option '-s' (default: None)" << "\n";
  cout << "\n";
